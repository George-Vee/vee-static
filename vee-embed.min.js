...
/* Seamless infinite marquee with clickable links */
(function(){
  const banner=document.getElementById('ve-win'); if(!banner) return;
  const base=banner.querySelector('.ve-wincont'); if(!base) return;

  banner.querySelector('.ve-wintrack')?.remove();
  const track=document.createElement('div'); track.className='ve-wintrack';

  const items=[...base.children].map(n=>n.cloneNode(true));

  function appendCycle(){ for(const n of items) track.append(n.cloneNode(true)); }
  base.remove(); banner.append(track);

  appendCycle(); appendCycle();

  function measureCycle(){
    let w=0,count=0;
    for(const el of track.children){ w+=el.getBoundingClientRect().width; if(++count===items.length) break; }
    const gap=parseFloat(getComputedStyle(track).gap||0);
    w += gap*Math.max(0,items.length-1);
    return Math.ceil(w);
  }
  function ensureLength(){
    const cycleW=measureCycle();
    while(track.scrollWidth < banner.clientWidth+cycleW) appendCycle();
    return cycleW;
  }

  let anim=null, PX_PER_SEC=60;
  function play(){
    anim?.cancel(); const cycleW=ensureLength(); if(!cycleW) return;
    track.style.width=track.scrollWidth+'px';
    anim=track.animate(
      [{transform:'translate3d(0,0,0)'},{transform:`translate3d(${-cycleW}px,0,0)`}],
      {duration:(cycleW/PX_PER_SEC)*1000,iterations:Infinity,easing:'linear'}
    );
  }
  function pause(){anim&&anim.pause()} function resume(){anim&&anim.play()}

  const reduce=matchMedia('(prefers-reduced-motion: reduce)');
  function apply(){ if(reduce.matches){anim?.cancel();track.style.transform='translate3d(0,0,0)';banner.style.overflowX='auto';}
    else{banner.style.overflowX='hidden';play();} }

  banner.addEventListener('mouseenter',pause);
  banner.addEventListener('mouseleave',resume);
  addEventListener('resize',()=>!reduce.matches&&play(),{passive:true});
  reduce.addEventListener?.('change',apply);

  Promise.resolve()
    .then(()=>document.fonts?.ready?.catch(()=>{}) ?? null)
    .then(()=>Promise.allSettled([...track.querySelectorAll('img')].map(img=>img.complete?null:new Promise(r=>img.addEventListener('load',r,{once:true})))))
    .then(apply);

  banner.veMarquee={refresh:play,pause,resume,setSpeed(pxps){if(pxps>0){PX_PER_SEC=pxps;play();}}};
})();
