...
/* Marquee WAAPI: seamless infinite, clickable */
(function(){
  "use strict";
  ...
  const appendCycle=()=>{ for(const n of items) track.append(n.cloneNode(true)); };
  ...
  const measureCycle=()=>{
    let w=0,c=0;
    for(const el of track.children){ w+=el.getBoundingClientRect().width; if(++c===items.length) break; }
    const gap=parseFloat(getComputedStyle(track).gap||0);
    return Math.ceil(w + gap*Math.max(0,items.length-1));
  };
  const ensureLength=()=>{
  ...
  let anim=null; let PX_PER_SEC=60;
  const play=()=>{
    anim?.cancel();
    const cycleW=ensureLength(); if(!cycleW) return;
  ...
  const pause=()=>{ if(anim) anim.pause(); };
  const resume=()=>{ if(anim) anim.play(); };

  const reduce=matchMedia('(prefers-reduced-motion: reduce)');
  const apply=()=>{
    if(reduce.matches){ anim?.cancel(); anim=null; track.style.transform='translate3d(0,0,0)'; banner.style.overflowX='auto'; }
    else { banner.style.overflowX='hidden'; play(); }
  };

  banner.addEventListener('mouseenter',pause);
  banner.addEventListener('mouseleave',resume);
  addEventListener('resize',()=>{ if(!reduce.matches) play(); },{passive:true});
